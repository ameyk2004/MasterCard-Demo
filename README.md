# Volunteer Management System for an NGO

```
An NGO, "Helping Hands," is struggling to manage its volunteers effectively. The organization needs a system that can help them assign tasks to volunteers, track their progress, and manage their availability. The system should also allow volunteers to view available tasks, register for tasks, and update their status. The NGO needs to ensure that only authorized personnel can create and assign tasks, while volunteers can only register for tasks when logged in.
```
